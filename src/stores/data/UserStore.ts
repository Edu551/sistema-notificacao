...
export const useUserStore = defineStore('UserStore', {
  ...
  getters: {
    getUserInfo: (state: InternalStoreStateType): UserModel => state.userInfo
  ...
  actions: {
    ...
    getUser: async function (email: string, password: string): Promise<void> {
      const service = new UserService('url/local');

      try {
        this.atribuirUser(await service.getUser(email, password));
      } catch {
        return;
      }
    }
